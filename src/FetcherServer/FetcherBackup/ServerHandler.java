	...
	private void signIn(IoSession session, String content) throws Exception {
		...
		try {
			if(!(instance.IsExist("user", "UserName", UserName))) {
				...
				instance.insertTableInfo("userinfo", insertuserinfo);
...
//				itemsInfo=itemsInfo+",\""+splashStr+"\"}";
	...
	// 接受带哥意向信息
		private void wantedInfo(IoSession session, Object message) throws Exception {
			...
			try {
				// 需要进行数据库合并测试！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
				int WantedID=instance.insertTableInfo("wantedInfo", data);
				// json格式，需修改
				// session.write(true);
				/*
				 * 没有错误就不返回 有错误就向服务器返回false
				 ...
				 * 核心函数
				 */
				matchAlreadyOrder(session,location,time,WantedID);
		...
		private void matchAlreadyOrder(
				IoSession session,String location ,String time ,int WantedID)
						throws Exception
		{
			try {
				...
				for(int i=0;i<wantMap.length;i++) {
					...
					if(!((destination.equals(location))&&(wantMap[i].get("ArriveTime").equals(time))&&(wantMap[i].get("State").equals(0)))) {
...
//				session.write(orders);
				...
				String cont="{\"type\":194,\"state\":0,"+"\"WantedID\":"+WantedID+",\"data\":[";
				if(j==0) {
					cont=cont+"]}";
					session.write(new MinaBean(cont));
					return;
	...
	private void acceptOrder(IoSession session,String content) throws Exception {
		...
		String fetcherId=data.getString("FetcherID");
		String WantedID=data.getString("WantedID");
		String BbId=null;
		//获取wantedid
		//int nu=instance.getNumber("wantedinfo", "FetcherID", fetcherId);
		//String dataofRow=instance.getRowofNumber("wantedinfo", "Fetcher", fetcherId, nu);
		//JSONObject dtaofrow=JSON.parseObject(dataofRow);
		//String WantedID=dtaofrow.getString("WantedID");
		...
		try {
			...
			//if(wantSession==null) {
				...
				instance.setTableInfo("wantedinfo","WantedID" , WantedID, "State", "1");
			//}
			//else {
				//197是提醒被接单代码
	...
	private void confirmTradeComplete(IoSession session,String content) throws Exception{
		...
		/*
		 * change the wantinfo state
		 */
		String wantidStr=instance.getTableInfo("traderecordinfo", "TradeID", TradeID, "WantID");
		JSONObject wantidObj=JSON.parseObject(wantidStr);
		String WantID=wantidObj.getString("WantID");
		instance.setTableInfo("wantinfo", "WantID", WantID, "State", "2");
		
		
		
		/*
		 * change the wantedinfo state
		 */
		String wantedidStr=instance.getTableInfo("traderecordinfo", "TradeID", TradeID, "WantedID");
		JSONObject wantedidObj=JSON.parseObject(wantedidStr);
		String WantedID=wantedidObj.getString("WantedID");
		instance.setTableInfo("wantedinfo", "WantedID", WantedID, "State", "2");
	 ...
	 * 115
	 */
	private void type115(IoSession session, String content) { 
		JSONObject messageObj = JSON.parseObject(content);
		String data = messageObj.getString("data");
	 ...
	 * 116
	 */
	private void type116(IoSession session, String content) { 
		JSONObject messageObj = JSON.parseObject(content);
		String data = messageObj.getString("data");
		JSONObject obj = JSONObject.parseObject(data);
		try {
			int tradeid = obj.getIntValue("TradeID");
			//int WantedID=obj.getIntValue("WantedID");
			Calendar a = Calendar.getInstance();
			SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String currenttime = df.format(a.getTime());
...
//			instance.setTableInfo("wantedinfo", "WantedID", WantedID, "State", 2);
	...
	private void type117(IoSession session, String content) { 
		JSONObject messageObj = JSON.parseObject(content);
		String data = messageObj.getString("data");
		JSONObject obj = JSONObject.parseObject(data);
		try {
	 ...
	 * 118
	 */
	private void type118(IoSession session, String content) { 
		JSONObject messageObj = JSON.parseObject(content);
		String data = messageObj.getString("data");
		JSONObject obj = JSONObject.parseObject(data);
		try {
